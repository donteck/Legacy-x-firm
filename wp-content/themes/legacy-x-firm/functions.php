...
function legacyx_enqueue_assets() {
    ...
    if (is_page('executive-assessment')) {
        wp_enqueue_style('legacyx-assessment', get_stylesheet_directory_uri() . '/assets/css/assessment.css', array('legacyx-polish'), file_exists($assessment) ? filemtime($assessment) : '1.0.0');
    }
}
...
function legacyx_client_portal_template($template) {
    ...
    if (is_page('executive-assessment')) {
        $assessment = get_theme_file_path('/executive-assessment.php');
        if (file_exists($assessment)) return $assessment;
    }
    return $template;
}
...
function legacyx_register_assessment_type() {
    register_post_type('legacyx_assessment', array(
        'labels' => array(
            'name' => 'Executive Assessments',
            'singular_name' => 'Executive Assessment',
            'menu_name' => 'Assessments',
            'all_items' => 'All Assessments',
            'view_item' => 'Review Assessment'
        ),
        'public' => false,
        'show_ui' => true,
        'show_in_menu' => true,
...
function legacyx_ensure_assessment_page() {
    ...
    if (!$page) {
        $page_id = wp_insert_post(array(
            'post_title' => 'Executive Assessment',
            'post_name' => 'executive-assessment',
            'post_status' => 'publish',
            'post_type' => 'page',
            'post_content' => ''
        ));
        if (!is_wp_error($page_id) && $page_id) update_option('legacyx_assessment_page_ready', 1, false);
    } else {
        update_option('legacyx_assessment_page_ready', 1, false);
    }
}
add_action('init','legacyx_ensure_assessment_page',20);

function legacyx_clean_assessment_array($values) {
...
function legacyx_submit_assessment() {
    if (!isset($_POST['legacyx_assessment_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['legacyx_assessment_nonce'])), 'legacyx_assessment_submit')) {
        wp_die('Security validation failed. Please return to the assessment and try again.', 'Legacy X Firm', array('response' => 403));
    }
    ...
    if (!$full_name || !is_email($email) || !$timeline || !$objective || !$consent) {
        wp_die('Please complete all required assessment fields.', 'Legacy X Firm', array('response' => 400));
    }

    $assessment_id = wp_insert_post(array(
        'post_type' => 'legacyx_assessment',
        'post_status' => 'private',
        'post_title' => $full_name . ' — ' . current_time('Y-m-d H:i')
    ));

    if (is_wp_error($assessment_id) || !$assessment_id) {
        wp_die('We could not save your assessment. Please try again.', 'Legacy X Firm', array('response' => 500));
    }
    ...
    $fields = array(
        ...
        'business_stage' => isset($_POST['business_stage']) ? sanitize_text_field(wp_unslash($_POST['business_stage'])) : '',
        'revenue' => isset($_POST['revenue']) ? sanitize_text_field(wp_unslash($_POST['revenue'])) : '',
        'capital_need' => isset($_POST['capital_need']) ? sanitize_text_field(wp_unslash($_POST['capital_need'])) : '',
        'timeline' => $timeline,
        'objective' => $objective,
        'obstacle' => isset($_POST['obstacle']) ? sanitize_textarea_field(wp_unslash($_POST['obstacle'])) : '',
        'engagement' => isset($_POST['engagement']) ? sanitize_text_field(wp_unslash($_POST['engagement'])) : '',
        'source' => isset($_POST['source']) ? sanitize_text_field(wp_unslash($_POST['source'])) : '',
        'priorities' => legacyx_clean_assessment_array(isset($_POST['priorities']) ? wp_unslash($_POST['priorities']) : array()),
        'consent' => 1,
...
add_action('admin_post_nopriv_legacyx_submit_assessment','legacyx_submit_assessment');
add_action('admin_post_legacyx_submit_assessment','legacyx_submit_assessment');

function legacyx_assessment_columns($columns) {
    return array(
        'cb' => isset($columns['cb']) ? $columns['cb'] : '<input type="checkbox" />',
        'title' => 'Principal',
        'business' => 'Business',
        'priority' => 'Primary Priorities',
        'timeline' => 'Timeline',
        'date' => 'Received'
    );
}
add_filter('manage_legacyx_assessment_posts_columns','legacyx_assessment_columns');

function legacyx_assessment_column_content($column, $post_id) {
    if ($column === 'business') echo esc_html(get_post_meta($post_id,'_legacyx_business_name',true));
    if ($column === 'priority') {
        $p = get_post_meta($post_id,'_legacyx_priorities',true);
        echo esc_html(is_array($p) ? implode(', ', array_slice($p,0,2)) : '');
    }
    if ($column === 'timeline') echo esc_html(get_post_meta($post_id,'_legacyx_timeline',true));
}
add_action('manage_legacyx_assessment_posts_custom_column','legacyx_assessment_column_content',10,2);

function legacyx_assessment_details_box() {
    add_meta_box('legacyx-assessment-details','Executive Assessment Details','legacyx_render_assessment_details','legacyx_assessment','normal','high');
}
add_action('add_meta_boxes','legacyx_assessment_details_box');

function legacyx_render_assessment_details($post) {
    $labels = array(
    ...
    echo '<div style="display:grid;grid-template-columns:220px 1fr;gap:0;border:1px solid #ddd">';
